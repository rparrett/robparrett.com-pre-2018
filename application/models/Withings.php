...
class WithingsModel {
	...
	function updateWeightsLocalFromRemote($userid) {
		...
		$sql = "INSERT INTO withings (userid, meastype, category, time, value) VALUES (:userid, :meastype, :category, :time, :value)";
		$stmt = $this->_db->prepare($sql);
	...
	function getWeightsLocal($userid) {
	...
	function getWeightsRemote($userid, $fromTimestamp) {
        ...
        foreach ($response->body->measuregrps as $group)  {
            ...
            foreach ($group->measures as $measure) {
				if ($measure->type != WithingsAPIClient::MEASTYPE_WEIGHT) continue;
	...
	function formatWeightsForHighCharts($weights) {
		$rows = array();
